...
def send_letter(
    ...
    # ALLOWED SENDERS - Add more roles as needed
    allowed_senders = ["Faculty Association", "Student", "SRC", "Staff"]
    if sender["role"] not in allowed_senders:
        conn.close()
        raise ValueError(f"Role '{sender['role']}' cannot send letters. Allowed roles: {', '.join(allowed_senders)}")
...
def act_on_letter(
    ...
    """Actor approves or rejects the current pending step for a letter.
...
def notify_sender(db_path: str, letter_id: int, reason: str) -> None:
    ...
    if row:
        ...
        # placeholder: print message or log
